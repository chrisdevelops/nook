...
describe("handlePause", () => {
  ...
  beforeEach(async () => {
    workdir = await mkdtemp(join(tmpdir(), "nook-pause-"));
  });
  ...
  test("pauses an active project for --days and records history", async () => {
    ...
    const result = await handlePause(
      { project: "alpha", days: 7, reason: "hands full" },
      ctx,
    );
    ...
    expect(metadata.paused_until).toBe(NOW + 7 * DAY_MS);
    ...
    expect(event.to).toBe("paused");
    expect(event.paused_until).toBe(NOW + 7 * DAY_MS);
    expect(event.reason).toBe("hands full");
    expect(event.at).toBe(NOW);
  });

  test("defaults --days to config.defaults.pause_max_days", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    const projectPath = await writeProject(
      rootDir,
      "active/alpha",
    ...
    const result = await handlePause({ project: "alpha" }, ctx);
    ...
    const metadata = JSON.parse(metadataRaw) as ProjectMetadata;
    expect(metadata.state).toBe("paused");
    expect(metadata.paused_until).toBe(NOW + 90 * DAY_MS);
  });

  test("accepts --until ISO date", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    const projectPath = await writeProject(
      rootDir,
      "active/alpha",
    ...
    // NOW = 2023-11-14T22:13:20.000Z. Pick any ISO datetime in the future.
    const untilIso = new Date(NOW + 30 * DAY_MS).toISOString();

    const result = await handlePause(
      { project: "alpha", until: untilIso },
      ctx,
    );
    ...
    const metadata = JSON.parse(metadataRaw) as ProjectMetadata;
    expect(metadata.state).toBe("paused");
    expect(metadata.paused_until).toBe(Date.parse(untilIso));
  });

  test("rejects --days and --until together", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "active/alpha",
    ...
    const result = await handlePause(
      {
        project: "alpha",
        days: 7,
        until: new Date(NOW + 30 * DAY_MS).toISOString(),
      },
      ctx,
    );
  ...
  test("rejects pausing an already-paused project", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "active/alpha",
    ...
    const result = await handlePause({ project: "alpha", days: 3 }, ctx);
  ...
  test("rejects pausing an archived project", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
  ...
  test("returns not_found when the project does not exist", async () => {
    ...
    const result = await handlePause({ project: "ghost", days: 3 }, ctx);
  ...
  test("leaves the project folder in place (no move)", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    const projectPath = await writeProject(
      rootDir,
      "active/alpha",
    ...
    const result = await handlePause({ project: "alpha", days: 5 }, ctx);
