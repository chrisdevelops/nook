...
describe("handleCd", () => {
  ...
  beforeEach(async () => {
    workdir = await mkdtemp(join(tmpdir(), "nook-cd-"));
  });
  ...
  test("prints the project's absolute path", async () => {
    ...
    const result = await handleCd({ project: "alpha" }, ctx);

    expect(result.ok).toBe(true);
    expect(info).toEqual([projectPath]);
  });

  test("returns not_found when no project matches", async () => {
    const { ctx } = await buildContext(workdir);

    const result = await handleCd({ project: "ghost" }, ctx);
