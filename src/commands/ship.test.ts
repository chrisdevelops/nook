...
describe("handleShip", () => {
  ...
  beforeEach(async () => {
    workdir = await mkdtemp(join(tmpdir(), "nook-ship-"));
  });
  ...
  test("ships an active project, moving it to <category>/shipped/<name>", async () => {
    ...
    await expect(access(join(rootDir, "work", "alpha", ".nook", "project.jsonc")))
      .rejects.toThrow();
    await access(
      join(rootDir, "work", "shipped", "alpha", ".nook", "project.jsonc"),
    );
    ...
    const historyRaw = await readFile(
      join(rootDir, "work", "shipped", "alpha", ".nook", "history.jsonl"),
      "utf8",
    );
  ...
  test("ships from paused, clearing paused_until", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "work/alpha",
    ...
    const result = await handleShip({ project: "alpha" }, ctx);
  ...
  test("ships from maintained", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "work/alpha",
  ...
  test("rejects shipping an already-shipped project", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "work/shipped/alpha",
  ...
  test("rejects shipping an archived project", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "work/archived/alpha",
  ...
  test("returns not_found when the project does not exist", async () => {
    ...
    const result = await handleShip({ project: "ghost" }, ctx);
