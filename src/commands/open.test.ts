...
describe("handleOpen", () => {
  ...
  beforeEach(async () => {
    workdir = await mkdtemp(join(tmpdir(), "nook-open-"));
  });
  ...
  test("invokes the platform opener with the project path", async () => {
    ...
    const spawn: LaunchSpawn = async (cmd) => {
      calls.push({ cmd });
      return { exitCode: 0 };
    };
    ...
    expect(calls).toEqual([{ cmd: ["xdg-open", projectPath] }]);
  });

  test("returns filesystem error when the opener fails", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "active/alpha",
    ...
    const spawn: LaunchSpawn = async () => ({ exitCode: 2 });
  ...
  test("returns not_found when no project matches", async () => {
    ...
    const result = await handleOpen({ project: "ghost" }, ctx);
