...
import { handleUnship } from "./unship.ts";
...
describe("handleUnship", () => {
  let workdir: string;

  beforeEach(async () => {
    workdir = await mkdtemp(join(tmpdir(), "nook-unship-"));
  });
  ...
  test("unships a shipped project back to <category>/<name> and sets state to active", async () => {
    ...
    const result = await handleUnship({ project: "alpha" }, ctx);
  ...
  test("rejects unshipping a non-shipped project", async () => {
    const { ctx, rootDir } = await buildContext(workdir);
    await writeProject(
      rootDir,
      "work/alpha",
  ...
  test("returns not_found when the project does not exist", async () => {
    ...
    const result = await handleUnship({ project: "ghost" }, ctx);
